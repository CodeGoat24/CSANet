...
from tool.utils import make_dirs
...
def infer(eval_loader, opt, model=None, repeat=1):
    ...
    assert (repeat >= 1)
    save_dir = os.path.join('/data/CodeGoat24/placement_result', opt.expid)
...
def parse_args():
    ...
    parser.add_argument("--epoch", type=int, default=9, required=False, help="which epoch to evaluate")
