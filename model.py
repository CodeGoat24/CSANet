...
from network import vgg16_bn, FgBgRegression, VAEEncoder, FeatExtractor, PoolingAttention, ConvMod, \
    PoolingAttentionBlock, ConvModBlock, PoolAttention, CrossConvAtt
...
class SysNet(nn.Module):
    def __init__(self, opt, img_size, init_weight=True):
        ...
        self.shared_net = nn.Sequential(*list(vgg16_bn(pretrained=True).features[:34]))#34
        self.fg_extractor = FeatExtractor(opt, n_mesh_list=[2, 4, 8], multi=True)
        self.bg_extractor = FeatExtractor(opt, n_mesh_list=[2, 4, 8], multi=True)
        self.latent_extractor = FeatExtractor(opt, n_mesh_list=[1], multi=False)
        self.vae_enc = VAEEncoder(opt)
        self.regress_net = FgBgRegression(opt)
        self.att = CrossConvAtt(opt);

        if init_weight:
            self.initialize_weight()

    def initialize_weight(self):
        for m in [self.fg_extractor, self.bg_extractor, self.latent_extractor, self.vae_enc, self.regress_net]:
    ...
    def forward(self, bg_img, fg_img, fg_msk, fg_bbox, comp_img=None, comp_msk=None):
        ...
        B, C, H, W = fg_feats_.shape
        fg_feats = self.fg_extractor(fg_feats_).unsqueeze(-1)
        bg_feats = self.bg_extractor(bg_feats_).unsqueeze(-1)


        attn_feats = self.att(fg_feats, bg_feats)

        randomness = torch.randn((attn_feats.shape[0], self.opt.d_noise)).cuda()


        trans_1_ = self.regress_net(torch.cat((attn_feats.squeeze(1), randomness), dim=1))
        ...
        latent_feats_ = self.latent_extractor(comp_feats_)

        latent_feats = latent_feats_.view(latent_feats_.shape[0], latent_feats_.shape[1], -1).transpose(1, 2).contiguous()
        latent_code, mu, logvar = self.vae_enc(latent_feats.squeeze(1))

        trans_2_ = self.regress_net(torch.cat((attn_feats.squeeze(1), latent_code), dim=1))
